...
merged <- merge(merged, T21BM_male04.seuratobj)
...
list <- SplitObject(merged, split.by = "T21.status")

anchors <- FindIntegrationAnchors(list, dims = 1:20, anchor.features = 20000)
...
umap.function.integrated <- function(x, dims, n.neighbors, min.dist, resolution) 
    ...
    x <- RunUMAP(x, reduction = "pca",  
                 n.neighbors = n.neighbors, 
                 min.dist = min.dist, 
                 dims = dims) 
    x <- FindNeighbors(x, reduction = 'umap', dims = 1:2)
...
save(seuratobj.integrated.umap, 
     file = '~/Desktop/ShortRead2024/R_objs/seuratobj_umap.RData')
```
...
# FindMarkers
```{r}
seurat.cluster.markers <- FindAllMarkers(seuratobj.integrated.umap)
