...
authors: "Rutendo Sigauke, Mary Allen"
date: "2025, 2026"
output:
...
```{r connect_google}
# 2. Request new auth with explicit scopes
gs4_auth(
...
url <- "https://docs.google.com/spreadsheets/d/1bDhGQ3uX6KrYJ2n4ra8v_ctzE64hHaP-rX4ri228rA8/edit?gid=0#gid=0"

# read as data.frame / tibble
mnm_df <- googlesheets4::read_sheet(url)

# if you want a data.table
mnm_dt <- data.table::as.data.table(mnm_df)
...
mnm_long_dt <- melt(mnm_dt, id.vars = c("Name","red_bowl"),
                    measure.vars =c('blue', 
                                    'brown', 'orange', 'yellow', 'red',
                                    'green',  'miniblue', 
                                    'minibrown', 'miniorange', 
                                    'miniyellow', 'minired','minigreen'))
...
colnames(mnm_long_dt) <- c("Name","red_bowl","candy_type","count")
...
```{r fold_change_red_red, echo = FALSE}

# 1: get value columns for just the M&Ms
mnm_redbowl_val_dt <- mnm_redbowl_dt[1,c('blue','brown','orange',
                                            'yellow','red','green')]

mnm_redbowl_val_dt2 <- mnm_redbowl_dt[2,c('blue','brown','orange',
                                              'yellow', 'red', 'green')]

# 2: calculate the fold change of the columns
mnm_redred_fc_dt <- mnm_redbowl_val_dt/mnm_redbowl_val_dt2
dim(mnm_redred_fc_dt)
mnm_redred_fc_dt
...
# 1: get value columns for just the M&Ms
mnm_greenbowl_val_dt <- mnm_greenbowl_dt[1,c('blue','brown','orange',
                                            'yellow','red','green')]

mnm_greenbowl_val_dt2 <- mnm_greenbowl_dt[2,c('blue','brown','orange',
                                              'yellow', 'red', 'green')]

# 2: calculate the fold change of the columns
mnm_greengreeen_fc_dt <- mnm_greenbowl_val_dt/mnm_greenbowl_val_dt2
dim(mnm_greengreeen_fc_dt)
mnm_greengreeen_fc_dt
...
mnm_greenbowl_val_dt <- mnm_redbowl_dt[2,c('blue','brown','orange',
                                              'yellow', 'red', 'green')]

# 2: calculate the fold change of the columns
mnm_redgreen_fc_single_dt <- mnm_redbowl_val_dt/mnm_greenbowl_val_dt
dim(mnm_redgreen_fc_single_dt)
mnm_redgreen_fc_single_dt
...
*Note:* The number of spike-in (miniMandMs) between the two bowls was kept constant.

```{r normalize_mnm_counts_spike, echo = FALSE}

# 1: we will get spike in totals for each sample (individual)
mnm_redbowl_spiketotal <- rowSums(mnm_redbowl_dt[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])
mnm_greenbowl_spiketotal <- rowSums(mnm_greenbowl_dt[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])

# 2: divide each row item with the spike in total
...
#ii- spike-in counts
mnm_redbowl_spiketotal_sum <- sum(mnm_redbowl_dt[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])
mnm_greenbowl_spiketotal_sum <- sum(mnm_greenbowl_dt[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])
...
#ii- spike-in counts
cntrl_redbowl_spiketotal_sum <- sum(mnm_cntrl_dtA[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])
cntrl_greenbowl_spiketotal_sum <- sum(mnm_cntrl_dtB[,c("miniblue","minibrown", "miniorange", "miniyellow", "minired", "minigreen")])
