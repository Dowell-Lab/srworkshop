...
setwd('/Users/rutendo/Desktop/SR2025/srworkshop/day06/results/')
getwd()
...
workdir <- '/Users/rutendo/Desktop/SR2025/srworkshop/day06/results/'
setwd(workdir)
getwd()

workdir2 = '/Users/rutendo/Desktop/SR2025/srworkshop/day06/results/'
setwd(workdir2)
getwd()
...
head(iris, 10) #see the first few rows for iris
tail(iris, 10)
...
paste0(workdir, 'iris_box_plot.png')
...
?paste0
paste(workdir, condy, '_vs_', condx, '/')
paste(workdir, condy, '_vs_', condx, '/', sep='')
...
##> The MPG and Weight are negatively correlated. 
##> The higher MPG cars have smaller WEIGHT
...
enviro_friendly <- subset(mtcars, mpg > 20) # we can consider 20 MPG as a good 
?subset
dim(enviro_friendly)
dim(mtcars)

#you want to look at how mpg and cylinders may be related. 
#You decide to make a new column that takes the RATIO of mpg to cylinder
head(enviro_friendly)
enviro_friendly$mpg_to_cylinder <- enviro_friendly$mpg/enviro_friendly$cyl
head(enviro_friendly)

#look at the data and see if there are any interesting trends 
#(hint: checking a scatter between mpg and mpg/cyl might be interesting)
plot(x=enviro_friendly$mpg,
     y=enviro_friendly$mpg_to_cylinder)

#now delete your new ratio column to restore mtcars to its original state
enviro_friendly <- enviro_friendly[-c(12)]
head(enviro_friendly) #confirming column was deleted

#rename the columns of mtcars to be more descriptive (like cyl is cylinder for example)
colnames(enviro_friendly) <- c("milespergal","cylinder","disp","horsepower",
                               "drat","weight","qsec","vs","am","gear","carb")
head(enviro_friendly)

#select mpg and cylinder columns only
head(enviro_friendly[,c("milespergal","cylinder")])

#transpose mtcars
enviro_friendly_t <- t(enviro_friendly)
dim(enviro_friendly_t)
head(enviro_friendly_t)
...
#What if we want significant genes that have a log2(fold change) of 
#MORE than 2 OR LESS than -2 (ie greater or less than a 2 fold change)
...
mtcars_filter_a <- subset(mtcars, wt >= 3.6 & mpg >= 15)
plot(mtcars_filter_a$wt, mtcars_filter_a$mpg)

#plot a histogram of weights for cars with a weigh less than 1.5 or greater than 3.6
mtcars_filter_b <- subset(mtcars, wt < 1.5 | wt > 3.6)
hist(mtcars_filter_b$wt)

#plot a scatter plot for mpg for this filtered dataframe
plot(mtcars_filter_b$wt, mtcars_filter_b$mpg)

#how many cars have exactly 4 cylinders? How many do NOT have 4 cylinders? 
#NOTE: you can use count() to count how many rows are in a dataframe.
count(subset(mtcars, cyl == 4))
count(subset(mtcars, cyl != 4))
table(mtcars$cyl) # another option is to use table

#how many cars have the same number of gears and carborators?

nrow(mtcars[mtcars$gear==mtcars$carb,])

#make mtcars car names into a column (hint: rownames)
mtcars$carnames <- rownames(mtcars)

#filter for all Toyta and Mazdas
mtcars[grep("Toyota|Mazda",  mtcars$carnames),]

#make a boxplot of mtcars gears where they are grouped by cylinder 
#(hint: you will need to use the Tilde (~) Operator if using base R boxplot function)
...
ggplot(mtcars, aes(x=wt, y=mpg)) + 
  ...
  labs(title = "MT Cars Weight and MPG", 
       y = "MPG",
       x = "Weight") +
  geom_hline(yintercept = 20, linetype='dashed', linewidth=1, color = 'gray20') +
  annotate("text", x = 4.75, y = 21, label = "Efficient cars", size = 6) +
  xlim(c(1,7))+ylim(c(0,35))+ 
  theme_cowplot(20) +
...
ggsave("mtcars_efficient_line.png", plot = last_plot(), path = workdir,
       scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
       dpi = 600, limitsize = TRUE)
