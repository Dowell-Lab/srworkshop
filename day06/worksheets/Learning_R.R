...
####### Your count tables (featureCounts output) and differential expression results (DESeq2 output) will be in the form of a dataframe.

#first let's do a quich check on what is in iris
...
#This is coorelated such that the longer the petal, the wider the petal
...
#this is useful for looking at differeces in iris species
...
v_line <- (mean(vir$petal_len) + mean(set$petal_len))/2 #calculating where to put the verticle line
...
#add a horizontal line for where "effecient" cars begin (judgement call- decide what makes a car effecient)
#label this new line on the plot "Effecient cars"
#save your plot
